...
void HAL_PCD_MspInit(PCD_HandleTypeDef *hpcd)
{
  ...
  if(hpcd->Instance == USB_OTG_FS)
  {
    ...
    GPIO_InitStruct.Pin = GPIO_PIN_11 | GPIO_PIN_12;    /*PA11=D- PA12=D+*/
	...
	/* Configure VBUS Pin */
    GPIO_InitStruct.Pin = GPIO_PIN_9;                   /*Vbus pin PA9*/
    GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

    /* This for ID line debug */
    GPIO_InitStruct.Pin = GPIO_PIN_10;                  /*ID line for auto detect host or device*/
...
uint8_t USBD_LL_IsStallEP(USBD_HandleTypeDef *pdev, uint8_t ep_addr)
{
  ...
  if((ep_addr & 0x80) == 0x80)                      /* check if highest bit is enabled 0x80=10000000*/
  {
    return hpcd->IN_ep[ep_addr & 0x7F].is_stall;    /* 0x7F=01111111*/
  }
  else
  {
